...
화면은 **음성 대화 프로세스 하나**로 통합되어 있습니다. STT·LLM·TTS 토글을 조합해 아래 기능을 사용합니다. STT를 끄면 텍스트 입력, LLM을 끄면 입력 내용 그대로 전달, TTS를 끄면 텍스트 출력으로 연결됩니다. 각 단계의 모델·옵션을 펼쳐 설정하고, 실행 후 단계별 상태·입력·출력·시간을 확인하세요. STT만 실행하거나 STT→TTS만 연결할 수도 있습니다.
